...
describe('getControlPoint', () => {
	it('should return null when currentPoints is not provided', () => {
		const result = getControlPoint(null as unknown as PointsArray, [0, 0], [1, 1], 0.5, false)
	...
	it('should handle first point in path by using current point as previous', () => {
		...
		const result = getControlPoint(current, null as unknown as PointsArray, next, smoothing, false)
		expect(result).not.toBeNull()
		if (result) {
			expect(result).toHaveLength(2)
		}
	...
	it('should handle last point in path by using current point as next', () => {
		...
		const result = getControlPoint(current, previous, null as unknown as PointsArray, smoothing, false)
		expect(result).not.toBeNull()
		if (result) {
			expect(result).toHaveLength(2)
		}
